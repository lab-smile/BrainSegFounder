...
simplified_data = [{"image": entry["image"], "label": entry["label"]} for entry in data]

# Number of repetitions for random shuffling and splitting
num_repeats = 5
...
for repeat in range(num_repeats):
    # Shuffle and split the data for this repetition
    ...
    fixed_test_set_file_path = f'./brats21_fewshot_fixed_test_set_repeat_{repeat + 1}.json'
    ...
    split_indices = [int(updated_total_pairs * perc / 100) for perc in [5, 10, 20, 40, 60, 80, 100]]

    # Create and save updated training subsets
    for perc, split_idx in zip([5, 10, 20, 40, 60, 80, 100], split_indices):
        subset_data = remaining_data[:split_idx]
        updated_train_subset_file_path = f'./brats21_fewshot_train_{perc}%_repeat_{repeat + 1}.json'
