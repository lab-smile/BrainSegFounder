...
def main():
        ...
        for step, batch in enumerate(train_loader):
            ...
            if args.rank == 0  and val_cond: #YY
                ...
                if val_loss_recon < val_best:
                    ...
                    save_ckp(checkpoint, args.logdir + "model_bestValRMSE.pt")
                    # print(
    ...
    def validation(args, test_loader, count_epoch, global_step):
        ...
        with torch.no_grad():
            for step, batch in enumerate(test_loader):
                ...
                loss_val.append(loss)              # YY loss_val.append(loss.item())
                loss_val_recon.append(loss_recon)  # YY loss_recon.item()
                ...
                print(f"[{args.rank}] " + "validation: " +
                      ...
                      f"Loss {loss.item():.4f}, " +  # YY
                      f"Loss Reconstruction {loss_recon.item():.4f}" # YY
                ) 
        ...
        return loss_val_mean.item(), loss_val_recon_mean.item(), img_list
    ...
    args.logdir = args.logdir + f"/run_{args.world_size:03d}G_" + datetime.now().strftime("%m-%d-%Y-%H:%M:%S") # YY 
