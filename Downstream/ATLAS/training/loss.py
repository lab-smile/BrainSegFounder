...
class Contrast(torch.nn.Module):
    ...
    def forward(self, x_i, x_j):
        ...
        sim = F.cosine_similarity(z.unsqueeze(1), z.unsqueeze(0),
                                  dim=2)  # matrix multiplication to obtain cosine similiarity: [batch_size x views, batch_size x views]
        sim_ij = torch.diag(sim,
                            self.batch_size)  # positive pairs are shifted from the main diagonal by views(=2), you will shift the main diagonal by views to obtain the similarity scores of positive pairs
        ...
        denom = self.neg_mask * torch.exp(
            sim / self.temp)  # this is to mask out the main diagonal, which is self-similarity all 1.
        return torch.sum(-torch.log(nom / torch.sum(denom, dim=1))) / (
                2 * self.batch_size)  # Final take logs of nom/sum(all similarity scores)
